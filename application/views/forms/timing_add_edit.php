       ...
       <legend><?php echo ($action == 'add') ? 'Add a new ' . $objScaffold->getName() : ucwords($action) . ' the ' . $objScaffold->getName() . ', <em>' . stripslashes($properties['title']) . '</em>'; ?></legend>
        ...
        <div class="field">
			...
			<select name="client" id="client">
				<option value="">Choose a client to filter the project list below</option>
			<?php echo drawDropDown(getDropDownOptions('client'), $objApplication->getParameter('client')); ?>
			</select>
		</div>
