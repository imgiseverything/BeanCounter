...
class Pension extends Scaffold{
	 ...
	 *	@var float
	 */
	protected $_taxReliefPercentage = 20;
	 ...
	 *	@return float
	 */
	protected function calcTaxRelief($amount){

		return ($amount * ($this->_taxReliefPercentage / 100));
	...
	public function setGrandTotal(){
		...
		if($this->_id){
			...
			$tax_relief = $this->calcTaxRelief($total);
		} else{
			...
			if(!empty($this->_properties)){
				...
				foreach($this->_properties as $property){
					...
					if(!empty($property['amount'])){
						...
						$tax_relief += $this->calcTaxRelief($property['amount']);
					}
					// put total value into relevant property array
					$this->_properties[$i]['total'] = $property['amount'];
					$this->_properties[$i]['tax_relief'] = $this->calcTaxRelief($property['amount']);
		...
		$this->_taxReliefTotal = $tax_relief;

		$this->_grandTotal = ($tax_relief + $total);
	 ...
	 *	getTaxReliefTotal()
	 */
	public function getTaxReliefTotal(){
		return $this->_taxReliefTotal;
	}
