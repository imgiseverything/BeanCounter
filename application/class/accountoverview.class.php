 ...
 */
	...
	class AccountOverview Extends Account{
		...
		protected function setMonths($filters){
			...
			//$start_date = date('F Y', strtotime($filters['timeframe_custom']['start']));
			//$end_date = date('F Y');
			...
			for($i = min($end_date_x, date('Ym')); $i >= $start_date_x; $i--){
				...
				$month = date('F', strtotime($year . '-' . $month_digits . '-01 00:00:00'));
			
			
				//echo (int)$month_digits .':' . $month . ' ' . $year . '<br>';
			
			
				if(((int)$month_digits < 12 && (int)$month_digits > 0) && empty($this->_months[$month . ' ' . $year])){
					array_unshift_associative($this->_months, $month . ' ' . $year, array());
				}
